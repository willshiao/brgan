# From https://raw.githubusercontent.com/snap-stanford/GraphRNN/master/create_graphs.py
import networkx as nx
import numpy as np
import random

from .graphrnn_utils import *
from .graphrnn_data import Graph_load, Graph_load_batch
...
def create(args):
    ...
    # real graphs
        ...
        for i in range(G.number_of_nodes()):
            ...
            if G_ego.number_of_nodes() >= 150 and (G_ego.number_of_nodes() <= 200):
                graphs.append(G_ego)
        ...
        graphs = []
        for i in range(G.number_of_nodes()):
            G_ego2 = nx.ego_graph(G, i, radius=2)
            if (G_ego2.number_of_nodes() >= 30) and (G_ego2.number_of_nodes() <= 50):
                graphs.append(G_ego2)
            G_ego3 = nx.ego_graph(G, i, radius=3)
            if (G_ego3.number_of_nodes() >= 30) and (G_ego3.number_of_nodes() <= 50):
                graphs.append(G_ego3)
        ...
        for i in range(G.number_of_nodes()):
            ...
            if G_ego.number_of_nodes() >= 150 and (G_ego.number_of_nodes() <= 200):
                graphs.append(G_ego)
        args['max_prev_node'] = 200
    elif args['graph_type'] == 'cora_small':
        _, _, G = Graph_load(dataset='cora')
        G = max((G.subgraph(c) for c in nx.connected_components(G)), key=len)
        G = nx.convert_node_labels_to_integers(G)
        graphs = []
